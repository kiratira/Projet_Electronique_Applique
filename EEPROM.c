...
unsigned char N = 0; //nbre d'enregistremet
...
void main(void) 
{
    ...
    Lecture_donnees_supp(); // on lit toute l'EEPROM pour voir si des data on été supp
    
    //tableau de test 
    unsigned char tableau[8] = {pos_segment[0],pos_segment[1],pos_segment[2],pos_segment[3],pos_segment[4],pos_segment[5],pos_segment[6],pos_segment[7]};
...
void Stockage_TAB_EEPROM(unsigned char TAB[8],unsigned char NB_elem) // TAB[8] = {int annee ,int mois , int jour  , int heure, int minute,int seconde,  int partie_entiere_survitesse, int partie_decimal_survitesse};
{
    ...
    {//Ecris les données dans l'EEPROM suivant le tableau d'adressage
        for(unsigned char i =0 ; i<NB_elem ; i++)
        {
...
void Lecture_EEPROM(unsigned char N, unsigned char Tab_transition [8]) // envoie les donné dans le tableau de transition de données
{
